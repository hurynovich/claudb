...
public final class ParamsScanner {
  ...
  /**
   * Checks if there are more parameters available and if the next parameter matches the given name.
   * In other words, this method checks that string representation of the next parameter is equal to the {@code name}.
   *
   * @param name value to be compared with the next parameter.
  ...
  /**
   * Checks if the next unprocessed parameter matches the given name.
   * It is similar to {@link #hasNext(String)}
   * but throws {@link CommandException} if there are no more parameters to process.
   *
   * @param name the name to compare with the next parameter
   * @return true if the next parameter matches the given name; false otherwise
   */
